...
class GPR_Matern:
    def __init__(self, xin, yin, nInput, nOutput, N, xlb, xub, alpha=1e-5, leng_s=1.0, leng_sb=[1e-1,10.0], nu=2.5):
        self.nInput  = nInput
        ...
        kernel = 1.0 * Matern(length_scale=leng_s, length_scale_bounds=(leng_sb[0], leng_sb[1]), nu=nu)

        smlist = []
        for i in range(nOutput):
            #smlist.append(GaussianProcessRegressor(kernel=kernel, alpha=1e-5, n_restarts_optimizer=5))
            smlist.append(GaussianProcessRegressor(kernel=kernel, alpha=alpha, optimizer=sceua_optimizer))
            smlist[i].fit(x,y[:,i])
        self.smlist = smlist

    def predict(self,xin,return_std=False):
        x = copy.deepcopy(xin)
        ...
        if not return_std:
            for i in range(self.nOutput):
                y[:,i] = self.smlist[i].predict(x,return_std=return_std)
            return y
        else:
            y_std = np.zeros((N,self.nOutput))
            for i in range(self.nOutput):
                y[:,i],y_std[:,i] = self.smlist[i].predict(x,return_std=return_std)
            return y,y_std
